...
import chatbotImg from "/images/chat/chatbot.png";
...
export default function ChatBot() {
    ...
    const initializeChat = async () => {
        try {
            ...
            const existingMessages: any = await getChatMessages(chatId);
        ...
        try {
            await sendMessage(content, currentChatId, {
                ...
                onSQL: (data: any) => {
                    ...
                    setMessages((prev: any) => [...prev, sqlMessage]);
                },
                onImage: (data: any) => {
                    const imageMessage = {
    ...
    if (isLoading) {
        return (
            <div className="bg-gray-50 flex items-center justify-center h-[550px] w-[400px] shadow-theme-lg rounded-[16px]">
                <Spin size="large" />
            </div>
        );
    }
    ...
    if (!user?.id) {
        return (
            <div className="flex flex-col bg-gray-50 h-[550px] w-[400px] shadow-theme-lg rounded-[16px] overflow-hidden">
