...
const FlightStatistic = ({ flight }: any) => {
    ...
    useEffect(() => {
        ...
        if (flight?.id) {
            serviceQuery += `&flight_id=${flight.id}`
        }
    ...
    }, [flight, selectedTime, JSON.stringify(selectedRangeTime)])
    ...
    return (
        <div>
            ...
            <div className="mt-[20px]">
                <TableCustomerBooking serviceType={SERVICE_TYPE.FLIGHT} flightId={flight?.id} minDate={selectedRangeTime.flight[0]} maxDate={selectedRangeTime.flight[1]} />
            </div>
        </div>
    )
}
