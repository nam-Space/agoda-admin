...
const HotelStatistic = ({ hotel }: any) => {
    ...
    useEffect(() => {
        ...
        if (hotel?.id) {
            serviceQuery += `&hotel_id=${hotel.id}`
        }
    ...
    }, [hotel, selectedTime, JSON.stringify(selectedRangeTime)])
    ...
    return (
        <div>
            ...
            <div className="mt-[20px]">
                <TableCustomerBooking serviceType={SERVICE_TYPE.HOTEL} hotelId={hotel?.id} minDate={selectedRangeTime.room[0]} maxDate={selectedRangeTime.room[1]} />
            </div>
        </div>
    )
}
