...
const ActivityDateStatistic = ({ activityDate }: any) => {
    ...
    useEffect(() => {
        ...
        if (activityDate?.id) {
            serviceQuery += `&activity_date_id=${activityDate.id}`
        }
    ...
    }, [activityDate, selectedTime, JSON.stringify(selectedRangeTime)])
    ...
    return (
        <div>
            ...
            <div className="mt-[20px]">
                <TableCustomerBooking serviceType={SERVICE_TYPE.ACTIVITY} activityDateId={activityDate?.id} minDate={selectedRangeTime.activity[0]} maxDate={selectedRangeTime.activity[1]} />
            </div>
        </div>
    )
}

export default ActivityDateStatistic