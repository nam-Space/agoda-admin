...
const ActivityStatistic = ({ activity }: any) => {
    ...
    useEffect(() => {
        ...
        if (activity?.id) {
            serviceQuery += `&activity_id=${activity.id}`
        }
    ...
    }, [activity, selectedTime, JSON.stringify(selectedRangeTime)])
    ...
    return (
        <div>
            ...
            <div className="mt-[20px]">
                <TableCustomerBooking serviceType={SERVICE_TYPE.ACTIVITY} activityId={activity?.id} minDate={selectedRangeTime.activity[0]} maxDate={selectedRangeTime.activity[1]} />
            </div>
        </div>
    )
}
