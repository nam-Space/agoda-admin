...
const ActivityPackageStatistic = ({ activityPackage }: any) => {
    ...
    useEffect(() => {
        ...
        if (activityPackage?.id) {
            serviceQuery += `&activity_package_id=${activityPackage.id}`
        }
        ...
        handleGetPaymentOverview(`booking__service_type=${SERVICE_TYPE.ACTIVITY}${serviceQuery}&statistic_by=${selectedTime}`)

    }, [activityPackage, selectedTime, JSON.stringify(selectedRangeTime)])
    ...
    return (
        <div>
            ...
            <div className="mt-[20px]">
                <TableCustomerBooking serviceType={SERVICE_TYPE.ACTIVITY} activityPackageId={activityPackage?.id} minDate={selectedRangeTime.activity[0]} maxDate={selectedRangeTime.activity[1]} />
            </div>
        </div>
    )
}

export default ActivityPackageStatistic