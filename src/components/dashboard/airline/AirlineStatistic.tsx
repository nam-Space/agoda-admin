...
const AirlineStatistic = ({ airline }: any) => {
    ...
    useEffect(() => {
        ...
        if (airline?.id) {
            serviceQuery += `&airline_id=${airline.id}`
        }
    ...
    }, [airline, selectedTime, JSON.stringify(selectedRangeTime)])
    ...
    return (
        <div>
            ...
            <div className="mt-[20px]">
                <TableCustomerBooking serviceType={SERVICE_TYPE.FLIGHT} airlineId={airline?.id} minDate={selectedRangeTime.flight[0]} maxDate={selectedRangeTime.flight[1]} />
            </div>
        </div>
    )
}

export default AirlineStatistic