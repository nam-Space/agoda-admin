...
const AircraftStatistic = ({ aircraft }: any) => {
    ...
    useEffect(() => {
        ...
        if (aircraft?.id) {
            serviceQuery += `&aircraft_id=${aircraft.id}`
        }
        ...
        handleGetPaymentOverview(`booking__service_type=${SERVICE_TYPE.FLIGHT}${serviceQuery}&statistic_by=${selectedTime}`)

    }, [aircraft, selectedTime, JSON.stringify(selectedRangeTime)])
    ...
    return (
        <div>
            ...
            <div className="mt-[20px]">
                <TableCustomerBooking serviceType={SERVICE_TYPE.FLIGHT} aircraftId={aircraft?.id} minDate={selectedRangeTime.flight[0]} maxDate={selectedRangeTime.flight[1]} />
            </div>
        </div>
    )
}

export default AircraftStatistic