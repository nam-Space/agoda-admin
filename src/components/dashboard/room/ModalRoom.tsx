...
import PhysicalRoomTable from "./PhysicalRoomTable";
...
const ModalRoom = (props: IProps) => {
    ...
    if (dataInit?.id) {
      ...
      const dataObj = {
        ...
        images: dataImages.map((image) =>
          (image.name as any)?.replaceAll(`${import.meta.env.VITE_BE_URL}`, "")
        ),
      };
  ...
  const handleRemoveFile = (file: any) => {
    const newDataImages = [...dataImages].filter(
      (image) => image.name !== file.url
    );
        ...
        <Row gutter={16}>
          ...
          <Col lg={6} md={6} sm={24} xs={24}>
            ...
            >
              <DebounceSelect
                ...
                fetchOptions={async () => {
                  return await Object.entries(STAY_TYPE_VI).map(
                    ([val, key]) => {
                      return {
                        label: key,
                        value: val,
          ...
          <Col lg={24} md={24} sm={24} xs={24}>
            <label className="flex items-center gap-[4px]">
              <span className="text-red-500 text-[20px]">*</span>Mô tả
            </label>
            <MDXEditor
