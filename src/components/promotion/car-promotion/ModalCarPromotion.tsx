...
import CarPromotionTable from "./CarPromotionTable";
import CarPromotionTableCreate from "./CarPromotionTableCreate";
...
const ModalCarPromotion = (props: IProps) => {
    ...
    const submitData = async (valuesForm: any) => {
        const { title, discount_percent, discount_amount, dates, is_active, promotion_type = PROMOTION_TYPE.CAR } = valuesForm;
        ...
        if (dataInit?.id) {
            ...
            const dataObj = {
                ...
                promotion_type,
                image: (dataImage[0]?.name as any)?.replaceAll(`${import.meta.env.VITE_BE_URL}`, ""),
                cars: carData.map((car: any) => {
                    return {
                        id: car.car_id,
                        discount_percent: car.discount_percent,
                        discount_amount: car.discount_amount
                    }
                })
            }
    ...
    return (
        ...
        >
            <Row gutter={16}>
                ...
                <Col lg={24} md={24} sm={24} xs={24}>
                    {dataInit?.id ? <CarPromotionTable promotion={dataInit} canCreate={true} canUpdate={true} canDelete={true} /> : <CarPromotionTableCreate carData={carData} setCarData={setCarData} />}
                </Col>
                <Col lg={24} md={24} sm={24} xs={24}>
...
export default ModalCarPromotion;
