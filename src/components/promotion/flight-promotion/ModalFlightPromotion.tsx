...
import FlightPromotionTable from "./FlightPromotionTable";
import FlightPromotionTableCreate from "./FlightPromotionTableCreate";
...
const ModalFlightPromotion = (props: IProps) => {
    ...
    const submitData = async (valuesForm: any) => {
        const { title, discount_percent, discount_amount, dates, is_active, promotion_type = PROMOTION_TYPE.FLIGHT } = valuesForm;
        ...
        if (dataInit?.id) {
            ...
            const dataObj = {
                ...
                promotion_type,
                image: (dataImage[0]?.name as any)?.replaceAll(`${import.meta.env.VITE_BE_URL}`, ""),
                flights: flightData.map((flight: any) => {
                    return {
                        id: flight.flight_id,
                        discount_percent: flight.discount_percent,
                        discount_amount: flight.discount_amount
                    }
                })
            }
    ...
    return (
        ...
        >
            <Row gutter={16}>
                ...
                <Col lg={24} md={24} sm={24} xs={24}>
                    {dataInit?.id ? <FlightPromotionTable promotion={dataInit} canCreate={true} canUpdate={true} canDelete={true} /> : <FlightPromotionTableCreate flightData={flightData} setFlightData={setFlightData} />}
                </Col>
                <Col lg={24} md={24} sm={24} xs={24}>
...
export default ModalFlightPromotion;
